...
const { createPetLink, THROTTLE_MS } =
...
test('running → done 仅显示完成气泡，不请求姿态切换', () => {
  const link = createPetLink();
  const m = mockPet();
  link.onSnapshot([row({ state: 'running' })], m, { now: T0, t });
...
// 完成动画已取消：旧动作名/白名单测试退役，姿态由 completion-pose-e2e 验证。
...
test('两个 done 同批：仅一条合并气泡', () => {
  ...
  assert.strictEqual(m2.calls.filter((c) => c[0] === 'playAnim').length, 0, '完成不切换姿态');
...
test('waiting + done 同批：waiting 是主角，仅 mixed 气泡', () => {
  const m = mockPet();
  const link = createPetLink();
  ...
  assert.strictEqual(m.calls.filter((c) => c[0] === 'playAnim').length, 0, '混合提醒也不切换姿态');
});
