...
- done → 仅 `pet.bubble('✅ <project> 的差事办完啦～')`，不主动切换宠物动画。2026-09-22 用户确认：后台完成提醒必须保留宿主的贴边探头、睡眠等当前行为；是否伴随说话动画由宿主自行仲裁。
- waiting → `pet.bubble('✋ <project> 在等你批准')`（橙语气，文案走 locale）。
- 去重：完成提醒按会话/回合只提醒一次，等待提醒保留 5 分钟节流；插件不读取宿主姿态，气泡表现由宿主仲裁。
- **同批合并（不轮流打扰）**：一轮 tick 内多个迁移只显示一条气泡，不请求动画——有 waiting 时它是主角
