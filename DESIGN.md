...
- 头部：标题「Agent 会话」+ 汇总胶囊 + 右侧 ⚙（设置）与 ✕（关面板）。**两个钮同规格**
  （2026-09-12 补入设计：Figma 头部只有 ✕，⚙ 是后加的）：命中区 22×22、图标 13×13、
...
- 会话行（圆角 12，左起）：
  1. **agent 徽标 26×26 圆角 8**：厂牌看主图标——Claude=陶土底白色官方星标 SVG；
     ...
     硬描反而失真。均打包本地资源，不引用远程。
     **右下角 13×13 形态角标**：底 Ink `#2A2E39` + `#3A3F4C` 描边（做在**外圈**：与自身同色的
     暗环压在 Codex 那张圆形渐变云上会成一坨黑块）、圆角 4.5，图形/文字**白色**。两档形态：
     CLI = `>_`（Inter Bold 6px，Figma 7:4）；App 任务（Codex App 与 WorkBuddy）= **窗口图形**
     （7×5.5 白色 1.2px 描边圆角 1.5 外框 + 顶部 7×1.6 白色实心标题栏，Figma 7:25）。
