...
// App thread ID, optionally followed by a resumed runtime thread ID.
// 形态不合就当没看见，绝不把脏字符串当 id 用。
const ROLLOUT_RE = /^rollout-.*-([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?:_([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}))?\.jsonl$/i;

// Share the same boundary check for activity and runtime metadata lookup. A suffix
// alone never authorizes looking up another task outside the indexed App path.
function readRolloutIdentity(home, file, expectedId, fsMod = fs) {
  try {
    const base = fsMod.realpathSync(path.join(home, 'sessions'));
    ...
    if (!match || (expectedId && match[1].toLowerCase() !== expectedId.toLowerCase())) return null;
    return { file: realFile, threadId: match[1].toLowerCase(), runtimeId: (match[2] || match[1]).toLowerCase() };
  } catch (_) { return null; }
}
...
module.exports = { createRolloutActivity, ACTIVE_WINDOW_MS, ROLLOUT_RE, readRolloutIdentity };
