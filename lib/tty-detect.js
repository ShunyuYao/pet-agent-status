...
//
// PET_AS_TTY 是测试隔离覆盖（同 PET_AS_* 约定，PROTOCOL.md 路径覆盖表）：tty 检测天生
// 依赖运行环境——同一份 hook 在开发机（有终端）与 CI（完全没有 tty）上会给出不同结果，
// 而 0.11.0「按落点过滤」之后这个差异会直接决定行显不显示（2026-09-12 CI 实际被它绊倒）。
// 需要确定性的用例用它把 tty 钉死；只认 `/dev/ttysNNN` 形态，别的一律无视。
function resolveTty(pid, opts) {
  const o = opts || {};
  const pinned = process.env.PET_AS_TTY;
  if (typeof pinned === 'string' && /^\/dev\/tty[a-z0-9]+$/i.test(pinned)) return pinned;
