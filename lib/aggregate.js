...
// 注意力优先级：等你批准 > 正在跑 > 出错了 > 刚办完 > 闲置/未知；同级取最新。
...
function pickFocus(rows) {
  ...
  for (const r of rows) {
    ...
    if (p == null) continue;
    if (!best || p < FOCUS_PRIORITY[best.state]
        || (p === FOCUS_PRIORITY[best.state] && r.ts > best.ts)) best = r;
  }
  return best;
}
...
module.exports = {
  ERROR_STALE_MS, STALE_UNKNOWN_MS, IDLE_MS, DROP_IDLE_MS, SUBLINE_KEY,
