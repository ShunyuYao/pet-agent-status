'use strict';
// 状态文件读写（PROTOCOL.md schema:3 的唯一实现处，兼容读取 schema:1/2）。
...
const STATES = ['running', 'waiting', 'waiting-input', 'done', 'ended', 'idle', 'failed', 'stopped'];
...
const ACTIVE_STATES = ['running', 'waiting', 'waiting-input'];
...
// 组装一条符合 schema:3 的记录。调用方给什么就写什么，缺省项补协议要求的 null。
function buildRecord(input) {
  ...
  if (typeof input.read === 'boolean') rec.read = input.read;
  if (typeof input.runId === 'string' && TURN_ID_RE.test(input.runId)) rec.runId = input.runId;
  return rec;
}
...
function writeUnlocked(input, dir) {
  ...
  // SessionStart is discovery, not evidence of a new task. An idle notification
  // has no turn identity and may be delayed; it cannot overwrite newer execution.
  if (existing && (rec.lastEvent === 'SessionStart' || rec.lastEvent === 'Notification' && rec.state === 'idle')) {
    return { file: fileFor(rec.sessionId, target), record: existing };
  }
  if (existing?.source === 'hook' && existing.lastEvent === 'SessionStart' && rec.lastEvent === 'SessionEnd') {
    removeStatus(rec.sessionId, target);
    return {file:fileFor(rec.sessionId, target),record:null};
  }
  if (existing && rec.lastEvent === 'SessionEnd' && ['done','failed','stopped'].includes(existing.state)) {
    return { file: fileFor(rec.sessionId, target), record: existing };
  }
  const newRun = rec.lastEvent === 'UserPromptSubmit'
    || (rec.turnId && rec.turnId !== existing?.turnId)
    || (ACTIVE_STATES.includes(rec.state) && !ACTIVE_STATES.includes(existing?.state));
  rec.runId = rec.turnId || rec.runId || (!newRun && existing?.runId) || randomUUID();
  if (existing?.turnId && rec.turnId === existing.turnId && ['done','failed','stopped'].includes(existing.state) && ACTIVE_STATES.includes(rec.state)) {
    return { file: fileFor(rec.sessionId, target), record: existing };
  }
  if (existing && existing.ts > rec.ts) return { file: fileFor(rec.sessionId, target), record: existing };
  carrySince(rec, newRun ? null : existing);
...
// Serialize writers per session. A crashed hook releases its lock when its PID
// is gone; a slow live writer is never evicted. Bounded waits fail quietly at the
// hook boundary instead of permitting an older read-modify-write to win.
function writeStatus(input, dir) {
  const target = dir || stateDir();
  fs.mkdirSync(target, {recursive:true});
  const lock = fileFor(input.sessionId, target) + '.lock';
  ...
  while (fd === undefined) {
    try { fd = fs.openSync(lock, 'wx'); fs.writeFileSync(fd, String(process.pid)); }
      ...
      if (fd !== undefined) {
        try { fs.closeSync(fd); } finally { try { fs.unlinkSync(lock); } catch (_) {} }
        throw error;
      }
      if (error.code !== 'EEXIST') throw error;
      // Re-read under a separate recovery guard: two observers of a dead
      // owner must not delete the replacement lock acquired by a live writer.
      const recovery = lock + '.reap';
      let recovering = false;
      try {
        fs.mkdirSync(recovery); recovering = true;
        const owner = Number(fs.readFileSync(lock, 'utf8'));
        if (Number.isInteger(owner) && owner > 0) {
          try { process.kill(owner, 0); }
          catch (probe) { if (probe.code === 'ESRCH') fs.unlinkSync(lock); }
        } else if (Date.now() - fs.statSync(lock).mtimeMs > 5000) fs.unlinkSync(lock);
      } catch (_) { /* another writer/recovery owns the path */ }
      finally { if (recovering) { try { fs.rmdirSync(recovery); } catch (_) {} } }
      if (Date.now() >= until) throw new Error('status-write-busy');
      Atomics.wait(sleeper, 0, 0, 10);
    }
  }
  try { return writeUnlocked(input, target); }
  finally { try { fs.closeSync(fd); } finally { try { fs.unlinkSync(lock); } catch (_) {} } }
}
...
function validateRecord(obj) {
  ...
  if (![1, 2, SCHEMA].includes(obj.schema)) {
  ...
  if (obj.read !== undefined && typeof obj.read !== 'boolean') return 'bad-read';
  if (obj.runId !== undefined && (typeof obj.runId !== 'string' || !TURN_ID_RE.test(obj.runId))) return 'bad-runId';
...
function readSnapshots(dir) {
  ...
  try {
    ...
    return { records, unknown, unknownCount: 0, unavailable: true };
  }
