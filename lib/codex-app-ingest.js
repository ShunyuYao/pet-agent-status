...
//   read-state hasUnreadTurn:true → 回合核验后 done（队列续聊竞态见 facts §12）
//   read-state hasUnreadTurn:false（用户在 App 读过了）→ ended，且**只更新不新建**——
//     对没见过的会话新建一条 ended 是在报旧闻
//
// 摄入保护：保留更富的数据源，并排除明确的子 Agent。
...
//   ③ Codex 明确标记的子 Agent 不落盘（身份由同一份只读线程元数据给出）。
...
function createCodexAppIngest(deps) {
  ...
  const pendingReadStates = new Map();
  ...
  function onRolloutActivity(conversationId, canClaim, thread = latestThread(conversationId)) {
    ...
    if (existing && (existing.state === 'done' || existing.state === 'ended')) {
      if (!turn || turn.status !== 'inProgress') return null;
  ...
  function onActivity(conversationId) {
    ...
    if (!ok) return null;
    const turn = thread?.turn;
    if (turn && (turn.status !== 'inProgress' || (existing && ['done', 'ended'].includes(existing.state) && existing.turnId === turn.id))) return null;
    return put(conversationId, 'running', 'ipc:queued-followups-changed', 'ipc', turn && turn.id);
  }
  ...
  function onReadState(conversationId, hasUnreadTurn, isActive) {
    ...
    if (!ok) return null;
    const turn = thread?.turn;
    const turnId = (turn && turn.id) || (existing && existing.turnId);
    // This broadcast has no turn ID. A queued turn can already be inProgress by
    // the time the previous completion arrives. Never seal that running turn.
    // Retain the signal so a delayed database projection can confirm completion
    // on a later poll, even after the rollout has stopped changing.
    if (hasUnreadTurn === true) {
      if (turn && !isTerminalTurn(turn)) {
        pendingReadStates.set(conversationId, { turnId: turn.id, hasUnreadTurn: true });
        return null;
      }
      if (!turn && pendingReadStates.has(conversationId)) {
        pendingReadStates.get(conversationId).hasUnreadTurn = true;
        return null;
      }
      pendingReadStates.delete(conversationId);
      return put(conversationId, 'done', 'ipc:turn-unread', 'ipc', turnId);
    }
    if (hasUnreadTurn === false && pendingReadStates.has(conversationId)) {
      pendingReadStates.get(conversationId).hasUnreadTurn = false;
      return reconcileReadState(conversationId, thread);
    }
    if (hasUnreadTurn === false && existing != null) {
      const runningTurn = turn && turn.status === 'inProgress';
      const terminalTurn = isTerminalTurn(turn);
      if (runningTurn && (existing.state === 'running' || turn.id !== existing.turnId)) return null;
      if (!runningTurn && !terminalTurn && existing.state === 'running'
          && typeof isActive === 'function' && isActive(conversationId)) return null;
      // 未知数据库状态不能使活动中的「已读」变成结束。
      return put(conversationId, 'ended', 'ipc:turn-read', 'ipc', existing.turnId || turnId);
    }
    return null;
  }

  function reconcileReadState(conversationId, thread) {
    const pending = pendingReadStates.get(conversationId);
    if (!pending) return null;
    const { existing, ok } = writable(conversationId);
    const turn = thread?.turn;
    if (!ok || thread?.isSubagent === true || (turn && turn.id !== pending.turnId)) {
      pendingReadStates.delete(conversationId);
      return null;
    }
    if (!isTerminalTurn(turn)) return null;
    const unread = pending.hasUnreadTurn;
    if (!unread && !existing) {
      pendingReadStates.delete(conversationId);
      return null; // Read notifications must not create previously unseen ended rows.
    }
    const result = put(conversationId, unread ? 'done' : 'ended', unread ? 'ipc:turn-unread' : 'ipc:turn-read', 'ipc', turn.id);
    if (result) pendingReadStates.delete(conversationId);
    return result;
  }

  return { onActivity, onReadState, onRolloutActivity, reconcileReadState,
    pendingIds: () => [...pendingReadStates.keys()], clearPending: () => pendingReadStates.clear(), APP_PROJECT };
}
