...
function createCodexAppIngest(deps) {
  ...
  const turnFor = typeof d.turnFor === 'function' ? d.turnFor : () => null;
  function latestTurn(id) { try { return turnFor(id); } catch (_) { return null; } }
   ...
   */
  function onRolloutActivity(conversationId, canClaim, turn = latestTurn(conversationId)) {
  ...
  function onActivity(conversationId) {
    ...
    const { existing, ok } = writable(conversationId);
    if (!ok) return null;
    const turn = latestTurn(conversationId);
  ...
  function onReadState(conversationId, hasUnreadTurn, isActive) {
    ...
    if (hasUnreadTurn === true) return put(conversationId, 'done', 'ipc:turn-unread', 'ipc', turnId);
    if (hasUnreadTurn === false && existing != null) {
      const runningTurn = turn && turn.status === 'inProgress';
      const terminalTurn = turn && ['completed', 'failed', 'interrupted'].includes(turn.status);
