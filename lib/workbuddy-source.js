...
const STATUS_MAP = {
  ...
  failed: { state: 'failed', updateOnly: true },
  error: { state: 'failed', updateOnly: true },
  terminated: { state: 'stopped', updateOnly: true },
  archived: { state: 'stopped', updateOnly: true }
...
function createWorkbuddySource(deps) {
  ...
  function queryRows() {
    ...
    try {
      ...
      const lookup = db.prepare('SELECT id,cwd,title,custom_title,status,updated_at,last_activity_at FROM sessions WHERE id=?');
      for (const record of stateFiles.readSnapshots(dir).records) {
        if (record.source !== 'poll' || record.agent !== 'workbuddy' || seen.has(record.sessionId)) continue;
        const row = lookup.get(record.sessionId);
  ...
  function tick() {
    ...
    for (const row of rows) {
      ...
      if (mapped.state === 'running' || mapped.state === 'waiting-input') {
        if (existing == null) {
