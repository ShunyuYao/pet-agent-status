...
// `/iTerm2/iTermServer-`：iTerm2 3.5+ 起会话由独立的 iTermServer 进程托管（会话恢复架构），
// 实测祖先链是 `iTermServer-3.6.11 → login → -zsh → claude`，**整条链上没有任何 `.app/` 片段**，
// 只认 `.app/` 会让现代 iTerm2 恒判 null、跳转入口永不出现（2026-09-10 真机抓到）。
const ITERM_MARKERS = ['/iTerm.app/', '/iTerm2.app/', '/iTerm2/iTermServer-'];
