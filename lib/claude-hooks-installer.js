...
// 卸载时据此决定该不该把 key 一并收回 —— 详见 CREATED_KEY 的用法注释。
...
// Claude Code 的 `hooks.<Event>[].hooks` 是数组，允许一组里挂多条命令；用户完全可能
// 把自己的命令追加进本插件写出的那个分组（配置里看着就是同一块，手改时最顺手）。
...
// 返回 { list, removed, createdKey, createdHooks }。install（幂等去重）与 uninstall
// 共用同一套摘除逻辑，避免两处粒度再次跑偏。
// createdKey / createdHooks：被摘掉的条目里标着「这个事件 key / hooks 容器是本插件造的」，
// 供 uninstall 决定收不收 key 与容器。
...
function install(options) {
  ...
  const hooksExisted = Object.prototype.hasOwnProperty.call(settings, 'hooks')
    && settings.hooks && typeof settings.hooks === 'object' && !Array.isArray(settings.hooks);
  if (!hooksExisted) settings.hooks = {};

  let added = 0;
  for (const event of HOOKED_EVENTS) {
    const existed = Object.prototype.hasOwnProperty.call(settings.hooks, event);
...
function uninstall(options) {
  ...
  for (const event of Object.keys(settings.hooks)) {
    ...
    if (res.createdKey && res.list.length === 0) delete settings.hooks[event];
    else settings.hooks[event] = res.list;
  }
  // hooks 容器同理：只有本插件造的容器才收回，用户原有的空 `hooks: {}` 原样还回去。
  if (createdHooks && Object.keys(settings.hooks).length === 0) delete settings.hooks;
