...
## 9. tty 到底准不准（2026-09-12 三组真机对照）

用户的顾虑很实在：tty 不是别人交给我们的，是**从进程树推出来的**（`lib/tty-detect.js`：
先看 hook 自己的 fd，再沿父链 `ps -o tty=` 上溯最多 6 层），而且这套逻辑有过前科——
注释里记着「线上 19 个状态文件里 18 个 tty 为 null」那次只上溯一层的缺陷。所以先量，再用。

做法：把 hook 的「嵌套检查」用一张只有 launchd 的假进程表关掉（`PET_AS_PS_OUTPUT`，
它只影响嵌套判定，tty 检测走自己的 ps 调用不受影响），然后在三种上下文里真跑 `claude -p`：

| 上下文 | 记录到的 tty | 判断 |
|---|---|---|
| ① 普通 `claude -p`（父进程是本会话，父有终端） | `/dev/ttys018`、pid 99593 | **继承了父会话的终端**（不准，但方向是"多给入口"） |
| ② `script -q /dev/null claude -p`（真伪终端） | `/dev/ttys029`、pid 56552 | **准**：拿到的是它自己的 pty，不是父的 ttys018 |
| ③ 完全无终端（launchd/Ralph 形态） | 真实数据佐证：Ralph 那批、meeting2task 那条、App 会话全是 `null` | **准**：稳定为 null |

结论：**tty 足够准，可以当过滤依据**——它唯一的失准方向是「无终端但祖先有终端时继承父的 tty」，
结果是**多给一个跳转入口**（fail-open），不会把真会话藏掉。而这个灰区最大的来源（嵌套
`claude -p`）已经在 0.10.2 于 hook 侧拦掉了。

② 这条尤其关键：它证明检测拿的是**该会话自己的** tty，而不是无脑取最近的祖先——
否则 ② 会记成父会话的 ttys018。

### 由此定下的过滤判据

**无 tty 且无 App 落点 → 不显示**（`lib/aggregate.js`，落点判定沿用已有的
`canJumpWithoutTty` 注入：Codex/WorkBuddy 深链接 || Claude App 归属）。

- 判据**不是** `canJump === false`：那一档还包含「有 tty 但认不出是哪个终端 App」
  （冷门终端、ps 缓存抖动），那种会话真实存在于某个终端里，按 canJump 过滤会误藏。
  **tty 非空一律保留。**
- 被隐藏的条数经 `summary.hiddenNoTarget` 如实报给面板，底部写明「另有 N 个后台会话未显示
  （没有可跳转的窗口）」——会话凭空消失比多显示一行更糟。
- 坏文件行（读不出来的状态文件）不受此规则影响：那是诊断信息，不是"点不进去的后台会话"。
